angular.module('app', ['ngStorage']).config(['$httpProvider', ($httpProvider) => {
    ...
    var isLogin = $sessionStorage.islogin !== undefined ? $sessionStorage.islogin : false; var service = 'Admin'; var data = { admin: { userName: null, password: null }, isLogin: isLogin, inquiries: null, services: null, initServise: null, loading: false }
    ...
    $scope.activeTab = 'inquery'; $scope.toggleTpl = (x) => { $scope.activeTab = x; switch (x) { case 'inquery': loadInquiries(); break; case 'services': loadServices(); break; default: } }
    ...
    InitServices(); $scope.addNewServiceGroup = (services) => {
    ...
    var service = 'Scheduler'; $scope.room = 0; var showScheduler = function () { YUI().use('aui-scheduler', function (Y) { var agendaView = new Y.SchedulerAgendaView(); var dayView = new Y.SchedulerDayView(); var weekView = new Y.SchedulerWeekView(); var monthView = new Y.SchedulerMonthView(); var eventRecorder = new Y.SchedulerEventRecorder({ on: { save: function (event) { addEvent(this.getTemplateData(), event); }, edit: function (event) { addEvent(this.getTemplateData(), event); }, delete: function (event) { removeEvent(this.getTemplateData(), event); } } }); new Y.Scheduler({ activeView: weekView, boundingBox: '#myScheduler', date: new Date(), eventRecorder: eventRecorder, items: $rootScope.events, render: true, views: [dayView, weekView, monthView, agendaView], strings: { agenda: 'Dnevni red', day: 'Dan', month: 'Mjesec', table: 'Tablica', today: 'Danas', week: 'Tjedan', year: 'Godina' }, }); }); }
    $scope.getSchedulerEvents = function (uid) { f.post(service, 'GetSchedulerEvents', { room: $scope.room, uid: uid }).then((d) => { $rootScope.events = d; $timeout(function () { showScheduler(); }, 200); }); }; $scope.getSchedulerEvents(null); var addEvent = function (x, event) { $rootScope.events.push({ room: $scope.room, clientId: null, content: event.details[0].newSchedulerEvent.changed.content, endDate: x.endDate, startDate: x.startDate, userId: null }); var eventObj = {}; eventObj.room = $scope.room; eventObj.clientId = null; eventObj.content = event.details[0].newSchedulerEvent.changed.content == null ? x.content : event.details[0].newSchedulerEvent.changed.content; eventObj.endDate = x.endDate; eventObj.startDate = x.startDate; eventObj.userId = null; var eventObj_old = {}; eventObj_old.room = $scope.room; eventObj_old.clientId = null; eventObj_old.content = angular.isUndefined(event.details[0].newSchedulerEvent.lastChange.content) ? x.content : event.details[0].newSchedulerEvent.lastChange.content.prevVal; eventObj_old.endDate = angular.isUndefined(event.details[0].newSchedulerEvent.lastChange.endDate) ? x.endDate : Date.parse(event.details[0].newSchedulerEvent.lastChange.endDate.prevVal); eventObj_old.startDate = angular.isUndefined(event.details[0].newSchedulerEvent.lastChange.startDate) ? x.startDate : Date.parse(event.details[0].newSchedulerEvent.lastChange.startDate.prevVal); eventObj_old.userId = null; remove(eventObj_old); $timeout(function () { save(eventObj); }, 500); }
    var save = function (x) { f.post(service, 'Save', { userGroupId: null, userId: null, x: x }).then((d) => { getAppointmentsCountByUserId(); }); }
    var removeEvent = function (x, event) { var eventObj = {}; eventObj.room = $scope.room; eventObj.clientId = null; eventObj.content = x.content; eventObj.endDate = x.endDate; eventObj.startDate = x.startDate; eventObj.userId = null; remove(eventObj); }
    var remove = function (x) { f.post(service, 'Delete', { userGroupId: null, userId: null, x: x }).then((d) => { getAppointmentsCountByUserId(); }); }
