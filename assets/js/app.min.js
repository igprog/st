angular.module('app', ['ngStorage', 'daypilot']).config(['$httpProvider', ($httpProvider) => {
    ...
    var isDelete = false; $scope.dpConfig = {
        ...
        }, onTimeRangeSelected: function (args) { var event = prompt("Opis:", ""); if (!event) return; var e = { id: DayPilot.guid(), text: event, start: args.start, end: args.end }; args.control.clearSelection(); $timeout(function () { $scope.events.push(e); save(e); }); }, onEventMoved: function (args) { var idx = $scope.events.findIndex(a=>a.id === args.e.data.id); $timeout(function () { save($scope.events[idx]); }); }, onBeforeEventRender: function (args) {
            args.data.areas = [{
                height: 17, width: 17, top: 3, right: 2, visibility: "Hover", cssClass: "event_action_delete", onClick: function (args) {
                    debugger; isDelete = true; if (!confirm("Želite li izbrisati ovaj termin?")) { args.preventDefault(); return; }
